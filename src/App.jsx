...
const ANOMALIES = [
  ...
  { id:"red_lights", name:"Czerwone światła", hint:"Światło przybrało krwisty kolor" },
  { id:"upside_down_jp2", name:"Odwrócony Papież", hint:"Obraz wisi do góry nogami" },
  { id:"giant_onion", name:"Gigantyczna Cebula", hint:"Zmutowana cebula pod drzwiami 7" },
  { id:"moving_stroller", name:"Jadący wózek", hint:"Uciekaj! Wózek cię goni!" },
  { id:"tv_noise", name:"Szum z TV", hint:"Telewizor stracił sygnał" },
  { id:"missing_exit", name:"Brak znaku", hint:"Znak wyjścia uciekł z sufitu" },
  { id:"blood_trail", name:"Ślady na dywanie", hint:"Dziwne, czerwone ślady ciągną się po korytarzu" },
  { id:"missing_doors", name:"Brak drzwi 5", hint:"Mieszkanie nr 5 po prostu zniknęło" },
  { id:"ceiling_eyes", name:"Oczy na suficie", hint:"Z ciemności na górze coś na ciebie patrzy" },
  { id:"all_doors_open", name:"Przeciąg", hint:"Wszystkie mieszkania są otwarte" },
  { id:"fake_tv_button", name:"Pułapka TV", hint:"Uderzenie w kineskop to był zły pomysł..." },
];
...
function mkWriting() {
  ...
  g.font="bold 42px monospace"; g.textAlign="center"; g.fillStyle="rgba(160,8,8,.95)"; g.fillText("JAM JEST 444",256,52);
  g.beginPath(); g.moveTo(55,52); g.lineTo(48,200); g.stroke();
  return c;
}
function mkNotice() {
  const c = document.createElement("canvas"); c.width=400; c.height=300; const g = c.getContext("2d");
  g.fillStyle="#E8E0C0"; g.fillRect(0,0,400,300);
  for(let i=0;i<200;i++){ g.fillStyle=`rgba(100,80,40,${Math.random()*.08})`; g.beginPath(); g.arc(Math.random()*400, Math.random()*300, Math.random()*15, 0, Math.PI*2); g.fill(); }
  ...
  lines.forEach((line, i) => {
    g.save();
    g.translate(200, 45 + i*50);
    g.rotate((Math.random() - 0.5) * 0.08); 
    g.font = `bold ${24 + Math.random()*6}px 'Comic Sans MS', cursive`;
    g.fillText(line, 0, 0);
    g.restore();
  });

  g.fillStyle="#A03030"; g.beginPath(); g.arc(20,20,6,0,Math.PI*2); g.fill(); g.beginPath(); g.arc(380,20,6,0,Math.PI*2); g.fill();
  return c;
}
...
function buildAptDoorOpen(num, style=0) {
  ...
  const thresh = new THREE.Mesh(BX(W_D+frameW*2, .04, .14), SM({color:0x5A3A18, roughness:.7})); thresh.position.set(0,-.02,.0); g.add(thresh);
  const voidMesh = new THREE.Mesh(new THREE.PlaneGeometry(W_D, H_D), new THREE.MeshBasicMaterial({color: 0x000000})); voidMesh.position.set(0, H_D/2, -0.05); g.add(voidMesh);

  const doorPivot = new THREE.Group(); doorPivot.position.set(W_D/2, 0, 0); doorPivot.rotation.y = 1.3; 
  const doorMat = SM({map:CT(mkDoor(num, style)), roughness:.75});
...
function buildEndingScene() {
    ...
    const groundTex = new THREE.TextureLoader().load('kostka.jpg');
    groundTex.wrapS = groundTex.wrapT = THREE.RepeatWrapping; groundTex.repeat.set(20, 20);
    const ground = new THREE.Mesh(new THREE.PlaneGeometry(100, 100), SM({map: groundTex, roughness: 0.9}));
    ground.rotation.x = -Math.PI/2; s.add(ground);

    const blokTex = new THREE.TextureLoader().load('blok.jpg');
    blokTex.wrapS = blokTex.wrapT = THREE.RepeatWrapping; blokTex.repeat.set(4, 4);
    const blokMat = SM({map: blokTex, roughness: 1.0});
    ...
    const maluchTex = new THREE.TextureLoader().load('maluch.png');
    const mMat = SM({color: 0xdd3333, map: maluchTex});
    const maluch = new THREE.Mesh(BX(1.4, 1.2, 3.0), mMat);
    maluch.position.set(0, 0.8, -10); maluch.rotation.y = 0.4;
    
    const tireMat = SM({color: 0x111111});
    [[-0.75, 0.3, 1.0], [0.75, 0.3, 1.0], [-0.75, 0.3, -1.0], [0.75, 0.3, -1.0]].forEach(pos => {
...
export default function PrlExit8() {
  ...
  const S = useRef({
    phase:"menu", 
    hasAnomaly:false, anomaly:null, activeAnomalies: [],
    exitCount:0, streak:0, sanity: 100, yaw:0, pitch:0,
    keys:{}, decided:false, tunnelLock:false,
    drag:false, dx:0, dy:0, _lastCatPrompt:"0", _lastRzepaHint: false,
    touchMove:{active:false,id:null,sx:0,sy:0},
    familiadaActive: false, hasRzepa: false, rzepaFound: false,
  ...
  useEffect(() => {
    ...
    if(R.current.audio) {
        if(R.current.audio.chopin) R.current.audio.chopin.muted = opts.muteMusic;
        if(R.current.audio.amb) R.current.audio.amb.muted = opts.muteSounds;
        if(R.current.audio.steps) R.current.audio.steps.muted = opts.muteSounds;
    }
  }, [opts]);
  ...
  useEffect(()=>{
    ...
    const audioChopin = new Audio("chopin-fin.m4a"); audioChopin.loop = true; audioChopin.volume = 0.4;
    const ambience = new Audio("freesound_community-horror-ambience-01-66708.mp3"); ambience.loop = true; ambience.volume = 0.35;
    const stepsAudio = new Audio("stomping-footsteps-sound-effect.mp3"); stepsAudio.loop = true; stepsAudio.volume = 0.55;
    
    R.current.audio = { chopin: audioChopin, amb: ambience, steps: stepsAudio };

    const startAudio = () => { 
        if(sr.phase === "menu") audioChopin.play().catch(()=>{}); 
    ...
    const onKD=e=>{
      if (e.code === 'KeyM') {
          if (sr.phase === "playing" && !sr.familiadaActive) {
              ...
              if(R.current.audio && R.current.audio.steps) R.current.audio.steps.pause();
    ...
    const darkRoom=new THREE.Mesh(BX(3.5,H,3.5),SM({color:0x010101,roughness:1,side:THREE.BackSide})); darkRoom.position.set(W/2+1.85,H/2,-25); scene.add(darkRoom);
    ...
    const notice=new THREE.Mesh(new THREE.PlaneGeometry(1.2,0.9),SM({map:CT(mkNotice())})); notice.rotation.y=-Math.PI/2; notice.position.set(W/2-.02,1.5,-4.5); scene.add(notice);
    ...
    const napisZrywajM = new THREE.Mesh(new THREE.PlaneGeometry(2.0,.45), new THREE.MeshBasicMaterial({map:CT(mkNapisZrywaj()),transparent:true,depthWrite:false})); napisZrywajM.rotation.y=Math.PI/2; napisZrywajM.position.set(-W/2+.04,2.35,-17); scene.add(napisZrywajM);
    ...
    const ao={ bloodTrail, sR, bigOnion, tvNoise };
    ...
    ceilingEyes.visible = false; scene.add(ceilingEyes); ao.ceilingEyes = ceilingEyes;
    ...
    function gen(){
      ...
      // Tryb Solo
          ...
          if (ha) {
              if (optsRef.current.hardcore) {
                  ...
                  activeAnoms = [ANOMALIES[Math.floor(Math.random() * ANOMALIES.length)]];
              }
          }
      }
      ...
      const hintText = activeAnoms.length > 0 ? activeAnoms.map(a => a.hint).join(" ORAZ ") : "";
      up({hasAnomaly:ha, hint:false, hintText: hintText, catPrompt:"", catMessage:"", tvPrompt: false, showHelp: true});
      ...
      activeAnoms.forEach(an => {
          switch(an.id){
            ...
            case"open_door": d7.visible=false; d7_open.visible=true; break;
            ...
            case"red_lights": lights.forEach(l=>{l.color.setHex(0xff0000);}); lamps.forEach(l=>{l.material.emissive.setHex(0xff0000);}); break;
            case"upside_down_jp2": jp2N.rotation.z = Math.PI; break;
            case"giant_onion": mess7.visible=false; ao.bigOnion.visible=true; break;
            ...
            case"all_doors_open": d7.visible=false; d7_open.visible=true; d3.rotation.y=-1.2; d5.rotation.y=1.2; d8.rotation.y=-1.2; break;
            case"fake_tv_button": break; 
          }
      });
    }
    R.current.gen=gen;
    ...
    function decide(action){
      ...
      if(ok){ 
          ...
          msg = action==="exit" ? (winCheck?"":"✓ Wyjście "+ne+(optsRef.current.endless?"":"/8")) : "👁 Dobrze! Wycofano się w porę. W."+ne+(optsRef.current.endless?"":"/8"); 
      } 
      else { 
      ...
      if(sr.phase==="playing" || sr.phase==="menu" || sr.phase==="paused" || sr.phase==="paused_options" || sr.phase==="busy" || sr.phase === "waiting_mp"){
        if (sr.phase === "playing") {
          ...
          const isObstacle = (px, pz) => {
              if (px > -1.85 && px < -0.45 && pz > -20.5 && pz < -19.5) return true; 
              if (sr.hasRzepa && !sr.rzepaFound) { if (px > -0.6 && px < 0.6 && pz > -8.6 && pz < -7.4) return true; }
              return false;
          };
          const isValidPos = (px, pz) => {
              if (px >= -1.35 && px <= 1.35 && pz >= -53.5 && pz <= 1.8) return true;
              if (px >= -7.5 && px < -1.35 && pz >= -1.8 && pz <= 1.8) return true;
              if (px >= -7.5 && px < -1.35 && pz >= -53.8 && pz <= -50.2) return true;
              return false;
          };
          let newX = cam.position.x + dx, newZ = cam.position.z + dz;
          if (isValidPos(newX, cam.position.z) && !isObstacle(newX, cam.position.z)) cam.position.x = newX;
          if (isValidPos(cam.position.x, newZ) && !isObstacle(cam.position.x, newZ)) cam.position.z = newZ;
          if (cam.position.x < -4.5 && !sr.decided && !sr.tunnelLock) { sr.tunnelLock = true; if (cam.position.z < -26) decide("exit"); else decide("back"); }
          ...
          if (sr.lastTvHit && Date.now() - sr.lastTvHit < 400) {
              ...
              [R.current.tvN, R.current.tvA, R.current.tvS, R.current.tvNoise].forEach(tv => { if(tv) { tv.rotation.z = (Math.random() - 0.5) * 0.1 * intensity; tv.rotation.x = (Math.random() - 0.5) * 0.1 * intensity; } });
          } else if (sr.lastTvHit) { sr.lastTvHit = 0; [R.current.tvN, R.current.tvA, R.current.tvS, R.current.tvNoise].forEach(tv => { if(tv) { tv.rotation.z = 0; tv.rotation.x = 0; } }); }
          ...
          if (R.current.ao?.cat?.visible) {
              ...
              if (dist < 2.5) {
                  if (sr._lastCatPrompt !== "1") { sr._lastCatPrompt = "1"; up({catPrompt: "Naciśnij interakcję aby pogłaskać kota"}); }
                  if (sr.keys['KeyE'] && !sr.familiadaActive) {
                      ...
                      setTimeout(() => up({catMessage: ""}), 4000); sr.keys['KeyE'] = false; 
                  }
          ...
          if (distToTv < 2.5) {
              ...
              if(sr.keys['KeyE']) {
                  ...
                  const hasTvTrap = sr.activeAnomalies.some(a => a.id === "fake_tv_button");
                  if (hasTvTrap) {
                      sr.sanity = Math.max(0, sr.sanity - 20); up({ sanity: sr.sanity, message: "⚠️ Uderzyłeś za mocno! Oszalałeś?", tvPrompt: false });
                      setTimeout(() => up(p => p.message?.includes("Oszalałeś") ? {...p, message:""} : p), 2000); sr.phase = "gameover_check";
                  } else { 
          ...
          if (R.current.ao?.sR?.visible) {
              ...
              if (sR.position.z < cam.position.z) sR.position.z += 0.035; else if (sR.position.z > cam.position.z) sR.position.z -= 0.035;
              const distToStroller = cam.position.distanceTo(sR.position);
              if (distToStroller < 1.5) {
                  const now = Date.now();
                  if (now - sr.lastHitTime > 800) {
                      sr.lastHitTime = now; sr.sanity = Math.max(0, sr.sanity - 15); up({ sanity: sr.sanity, message: "⚠️ WÓZEK CIĘ ATAKUJE! WIEJ!" });
                      setTimeout(() => up(p => p.message?.includes("WÓZEK") ? {...p, message:""} : p), 800); sr.pitch = Math.min(0.52, sr.pitch + 0.2);
                      if (sr.sanity <= 0) { sr.phase = "gameover"; up({ phase: "gameover", sanity: sr.sanity, exitCount: sr.exitCount, streak: sr.streak, steps: stepCount.current }); }
                  }
              }
          }
        ...
        if(R.current.rzepaM){
          ...
          if(sr.hasRzepa && !sr.rzepaFound && sr.phase === "playing"){
            ...
            if(dist < 2.5 && sr.keys['KeyE'] && !sr.familiadaActive){
              ...
              up({ rzepaHint: false, tvPrompt: false, familiada: { active: true, qId: Math.floor(Math.random()*FAMILIADA_Q.length), step: 'question', points: 0, ansText: "" } });
            }
    ...
    return()=>{
      ...
      if(R.current.audio && R.current.audio.chopin){R.current.audio.chopin.pause();R.current.audio.chopin=null;}
    };
  },[]); 

  const submitFamiliada = () => {
      if(!famInput.trim()) return;
      const sr = S.current; const textVal = famInput.trim().toLowerCase(); const q = FAMILIADA_Q[ui.familiada.qId];
      ...
      setFamInput(""); 
      setTimeout(() => { sr.familiadaActive = false; setUi(prev => ({ ...prev, familiada: { active: false, qId: 0, step: 'none', points: 0, ansText: "" } })); }, 4000);
  };

  const startActualGame = (isMultiplayer, serverData = null) => {
      const sr = S.current;
      sr.phase = "busy";
      if(R.current.audio && R.current.audio.chopin) R.current.audio.chopin.pause();
      ...
      sr.exitCount = 0; sr.streak = 0; sr.sanity = 100;
    ...
    <div style={{width:"100%",height:"100vh",background:"#000",overflow:"hidden",position:"relative",fontFamily:"'Courier New',monospace",userSelect:"none",touchAction:"none"}}>
      ...
      {phase==="options" && (
          <div style={{position:"absolute", inset:0, zIndex: 100, background:"#0a0a0a", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", overflowY:"auto", padding: "40px 0"}}>
              ...
              <div style={{display:"flex", flexDirection:"column", gap:"15px", width: "350px", marginBottom: "40px"}}>
                  ...
                  <label style={{color:"#fff", fontSize:"14px", display:"flex", justifyContent:"space-between", alignItems:"center", cursor:"pointer"}}>
                      Efekt Kasety VHS
                      <input type="checkbox" checked={opts.vhs} onChange={(e)=>setOpts(p=>({...p, vhs: e.target.checked}))} style={{transform:"scale(1.5)"}}/>
                  </label>
                  <label style={{color:"#fff", fontSize:"14px", display:"flex", justifyContent:"space-between", alignItems:"center", cursor:"pointer"}}>
                      Wycisz muzykę (Menu)
                      <input type="checkbox" checked={opts.muteMusic} onChange={(e)=>setOpts(p=>({...p, muteMusic: e.target.checked}))} style={{transform:"scale(1.5)"}}/>
                  </label>
                  <label style={{color:"#fff", fontSize:"14px", display:"flex", justifyContent:"space-between", alignItems:"center", cursor:"pointer"}}>
                      Wycisz dźwięki (Gra)
                      <input type="checkbox" checked={opts.muteSounds} onChange={(e)=>setOpts(p=>({...p, muteSounds: e.target.checked}))} style={{transform:"scale(1.5)"}}/>
                  </label>
              </div>
              <button onClick={()=>setUi(p=>({...p, phase:"menu"}))} style={{...btnStyle, width:"200px"}}>WRÓĆ</button>
          </div>
      )}
      ...
      {phase==="paused" && (
          <div style={{position:"absolute", inset:0, zIndex: 100, background:"rgba(0,0,0,0.85)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center"}}>
              <h2 style={{color:"#d8b860", fontSize:"48px", marginBottom:"40px", textShadow:"0 0 20px rgba(216,184,96,0.6)"}}>PAUZA</h2>
              <div style={{display:"flex", flexDirection:"column", gap:"20px", width: "250px"}}>
                  <button onClick={() => { S.current.phase = "playing"; setUi(p=>({...p, phase:"playing"})); }} style={btnStyle}>WZNÓW</button>
                  <button onClick={() => setUi(p=>({...p, phase:"paused_options"}))} style={{...btnStyle, borderColor:"#555", color:"#aaa"}}>OPCJE</button>
                  <button onClick={restart} style={{...btnStyle, borderColor:"#a82020", color:"#a82020"}}>WYJDŹ DO MENU</button>
              </div>
          </div>
      )}
      ...
      {phase==="paused_options" && (
          <div style={{position:"absolute", inset:0, zIndex: 100, background:"#0a0a0a", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", overflowY:"auto", padding: "40px 0"}}>
              ...
              <div style={{display:"flex", flexDirection:"column", gap:"15px", width: "350px", marginBottom: "40px"}}>
                  ...
                  <label style={{color:"#fff", fontSize:"14px", display:"flex", justifyContent:"space-between", alignItems:"center", cursor:"pointer"}}>Wycisz dźwięki (Gra)<input type="checkbox" checked={opts.muteSounds} onChange={(e)=>setOpts(p=>({...p, muteSounds: e.target.checked}))} style={{transform:"scale(1.5)"}}/></label>
              </div>
              <button onClick={()=>setUi(p=>({...p, phase:"paused"}))} style={{...btnStyle, width:"200px"}}>WRÓĆ</button>
          </div>
      )}
      ...
      {(phase==="playing" || phase==="paused" || phase==="paused_options") && <>
        {!isMobile && (
          <div style={{ position:"absolute", bottom:20, left:20, zIndex:50, background:"rgba(0,0,0,0.6)", border:"1px solid rgba(200,168,64,0.3)", color:"#aaa", padding:"10px 15px", borderRadius:"4px", fontSize:"11px", lineHeight:"1.6", pointerEvents:"none" }}>
            <span style={{color:"#d8b860"}}>Ruch</span> = W, A, S, D<br/>
            <span style={{color:"#d8b860"}}>Interakcje</span> = E<br/>
            <span style={{color:"#d8b860"}}>Sprint</span> = SHIFT<br/>
            <span style={{color:"#d8b860"}}>Latarka</span> = F
          </div>
        )}
        <div style={{position:"absolute", zIndex: 50, top:12,left:"50%",transform:"translateX(-50%)",display:"flex",gap:5}}>
          {opts.endless ? (
             <div style={{color:"#c8a840", fontSize:"14px", fontWeight:"bold"}}>W.{exitCount}</div>
          ) : (
        ...
        {showHelp && (
          <div style={{position:"absolute", zIndex: 50, bottom: isMobile ? "auto" : 30, top: isMobile ? 80 : "auto", width:"100%", textAlign:"center", pointerEvents:"none", transition: "opacity 1s ease-out"}}>
            <div style={{display:"inline-block", background:"rgba(0,0,0,0.8)", border:"1px solid rgba(200,168,64,0.4)", color:"#d8b860", padding:"8px 16px", borderRadius:"4px", fontSize:isMobile?"11px":"13px", letterSpacing:"1px"}}> Brak anomalii ➔ idź naprzód &nbsp;&nbsp;|&nbsp;&nbsp; Anomalia ➔ wycofaj się {!isMobile && <span style={{color:"#888", marginLeft:"15px"}}>[M] Menu</span>} </div>
          </div>
        )}
        ...
        {tvPrompt&&<div style={{position:"absolute", zIndex: 50, top: isMobile ? 120 : "auto", bottom: isMobile ? "auto" : 85, left:"50%",transform:"translateX(-50%)",background:"rgba(0,0,0,.85)",border:"1px solid #a86020",color:"#d8b860",padding:"7px 20px",borderRadius:2,fontSize:"11px",letterSpacing:"1px",whiteSpace:"nowrap"}}> 📺 Naciśnij <strong>{isMobile ? "[🖐️]" : "[E]"}</strong> aby uderzyć w kineskop </div>}
        ...
        {ui.rzepaHint && !ui.familiada.active && ( <div style={{position:"absolute", zIndex: 50, top: isMobile ? 150 : "auto", bottom: isMobile ? "auto" : 120, left:"50%",transform:"translateX(-50%)",background:"rgba(0,0,0,.85)",border:"1px solid #c8a840",color:"#d8b860",padding:"6px 18px",borderRadius:3,fontSize:"11px",letterSpacing:"2px",whiteSpace:"nowrap"}}> 🌱 Zagraj w familiadzie - użyj <strong>{isMobile ? "[🖐️]" : "[E]"}</strong> </div> )}
          ...
          <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.9)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:100}}>
             {familiada.step === 'question' && (
                 <div style={{display:"flex", flexDirection:"column", alignItems:"center"}}>
                     <div style={{color:"#fff", fontSize:"20px", marginBottom:"20px", textAlign:"center", maxWidth:"600px", textShadow:"0 0 10px #fff"}}> Karol pyta: "{FAMILIADA_Q[familiada.qId].q}" </div>
                     <div className="fam-board"> {Array.from({length: 5}).map((_, i) => { const ans = FAMILIADA_Q[familiada.qId].a[i]; const isRevealed = familiada.step === 'result' && familiada.ansText.toLowerCase() === (ans ? ans.t.toLowerCase() : ""); let rowText = "..................", rowPts = "--"; if (ans && isRevealed) { rowText = ans.t.toUpperCase().padEnd(18, ' '); rowPts = ans.p.toString().padStart(2, '0'); } else if (!ans) { rowText = "                  "; rowPts = "  "; } return ( <div key={i} className="fam-text" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}> <div style={{ display: 'flex', gap: '15px' }}> <span>{ans ? i+1 : '\u00A0'}</span> <span style={{ whiteSpace: 'pre' }}>{rowText}</span> </div> <span>{rowPts}</span> </div> ); })} </div>
                     <div style={{display:"flex", flexDirection:"row", gap:"10px", width:"100%", maxWidth:"600px", justifyContent:"center"}}>
                         <input type="text" autoFocus value={famInput} onChange={e => setFamInput(e.target.value)} onKeyDown={e => { e.stopPropagation(); if(e.key === 'Enter') submitFamiliada(); }} placeholder="Wpisz odpowiedź..." style={{padding:"12px", fontSize:"18px", width:"70%", borderRadius:"4px", border:"2px solid #555", background:"#111", color:"#fff", outline:"none", textAlign:"center", fontFamily: 'monospace'}} />
                         <button onClick={submitFamiliada} style={{background:"#4CAF50", border:"none", color:"#fff", padding:"10px 20px", fontSize:"16px", cursor:"pointer", borderRadius:"4px", fontWeight:"bold"}}> STRZAŁ </button>
                     </div>
                 </div>
             )}
             {familiada.step === 'result' && (
                 <div style={{display:"flex", flexDirection:"column", alignItems:"center"}}>
                     ...
                     <div className="fam-board"> {Array.from({length: 5}).map((_, i) => { const ans = FAMILIADA_Q[familiada.qId].a[i]; const isRevealed = familiada.ansText.toLowerCase() === (ans ? ans.t.toLowerCase() : ""); let rowText = "..................", rowPts = "--"; if (ans && isRevealed) { rowText = ans.t.toUpperCase().padEnd(18, ' '); rowPts = ans.p.toString().padStart(2, '0'); } else if (!ans) { rowText = "                  "; rowPts = "  "; } return ( <div key={i} className="fam-text" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}> <div style={{ display: 'flex', gap: '15px' }}> <span>{ans ? i+1 : '\u00A0'}</span> <span style={{ whiteSpace: 'pre' }}>{rowText}</span> </div> <span>{rowPts}</span> </div> ); })} </div>
                     <div style={{background:"#0a0f18", border:"3px solid #f0b000", borderRadius:"12px", padding:"20px", textAlign:"center", maxWidth:"400px", zIndex: 105}}>
                         <img src="/strasburger.jpg" alt="Prowadzący" style={{width:"100%", borderRadius:"8px", marginBottom:"15px", border:"2px solid #555"}} />
                         <div style={{color: familiada.points === 100 ? "#4CAF50" : (familiada.points > 0 ? "#f0b000" : "#d32f2f"), fontSize:"22px", fontWeight:"bold", marginBottom:"10px"}}> {familiada.points === 100 ? "100% - Najwięcej odpowiedzi!" : (familiada.points > 0 ? `Dobra odpowiedź! To punktuje!` : "Żaden ANKIETOWANY nie odpowiedział w ten sposób!")} </div>
                         <div style={{color:"#fff", fontSize:"16px", marginBottom:"15px"}}> {familiada.points > 0 && <span>Odzyskujesz <span style={{color:"#f0b000", fontWeight:"bold"}}>{familiada.points}%</span> poczytalności.</span>} </div>
                     </div>
                 </div>
             )}
          </div>
        ...
        <div style={{position:"absolute", zIndex: 50, top: isMobile ? 12 : 40, left:"50%", transform:"translateX(-50%)", textAlign:"center"}}>
          <button onClick={()=>setUi(p=>({...p,hint:!p.hint}))} style={{background:"none",border:"none",color:"#3a2e1c",cursor:"pointer",fontSize:"9px",letterSpacing:"2px",padding:"4px 8px"}}> {hint?"UKRYJ PODPOWIEDŹ":"POKAŻ PODPOWIEDŹ"} </button>
          {hint&&<div style={{color:"#6a5838",fontSize:"10px",fontStyle:"italic",marginTop:3,letterSpacing:".5px", background:"rgba(0,0,0,0.5)", padding:"2px 6px", borderRadius:"2px"}}> {hasAnomaly?`⚠ ${hintText}`:"Wszystko wygląda normalnie..."} </div>}
        </div>
      </>}
      ...
      {phase==="win"&&(
        <div style={{position:"absolute", zIndex: 100, inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-end", padding:"0 0 40px"}}>
          <div style={{background:"rgba(0,0,0,.85)",padding:"24px 40px",textAlign:"center",borderTop:"2px solid rgba(200,168,64,.3)",width:"100%"}}>
            ...
            <div style={{color:"#d8b860",fontSize:isMobile?"16px":"20px",letterSpacing:"4px",marginBottom:8}}>WOLNOŚĆ!</div>
            <div style={{color:"#aaa",fontSize:"13px",fontStyle:"italic",marginBottom:4}}>"Może i szaro, ale chociaż na zewnątrz..."</div>
            <div style={{color:"#555",fontSize:"10px",marginBottom:16}}>Kroków: {steps} — Zakończenie</div>
            <button onClick={restart} style={btnStyle}>WRÓĆ DO MENU</button>
          </div>
        </div>
      )}
      ...
      {phase==="gameover"&&(
        <div style={{position:"absolute", zIndex: 100, inset:0,background:"rgba(10,0,0,.96)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14}}>
          ...
          {opts.endless && <div style={{color:"#d8b860",fontSize:"14px", marginTop: "10px"}}>Dotarłeś do klatki: {exitCount}</div>}
